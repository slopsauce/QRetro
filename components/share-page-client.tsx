...
export function SharePageClient() {
  ...
  useEffect(() => {
    const parseFragment = () => {
      ...
      if (fragment) {
        // Simple manual parsing for iOS compatibility
        const params: Record<string, string> = {};
        fragment.split('&').forEach(pair => {
          const [key, ...values] = pair.split('=');
          if (key) params[key] = values.join('=');
        });
        setType(params.type || null);
        setData(params.data || null);
      }
    };

    // Parse on mount with small delay for iOS
    setTimeout(parseFragment, 50);
